 ...
 */
function generateTodoHash(path, todoContent) {
 ...
 */
export async function processCommits(context, repo, commits) {
    ...
    for (const commit of commits) {
        ...
        await Promise.all(files.map(path => processFile(context, repo, commit, path, config)));
    }
}
/**
 ...
 */
async function processFile(context, repo, commit, path, config) {
    try {
 ...
 */
async function processTodosInFile(context, repo, commit, path, lines, fileContent, config) {
    ...
    for (let index = 0; index < lines.length; index++) {
        ...
        if (config.todoMarkers.some(marker => line.includes(`// ${marker}`))) {
            ...
            const issueTitle = todoMessage
                ? `${todoMessage} in ${path}`
                : `TODO found in ${path}`;
            await createIssueIfNotExists(context, repo, issueTitle, path, line, index, commit);
        }
    }
    ...
    while ((blockMatch = todoBlockPattern.exec(fileContent)) !== null) {
        ...
        const issueTitle = todoMessage
            ? `${todoMessage} in ${path}`
            : `TODO found in ${path}`;
        await createIssueIfNotExists(context, repo, issueTitle, path, fullMatch, lineNumber - 1, commit, true);
    }
            ...
            if (line.startsWith("//") &&
                ...
                if (multiLineTodoContent.includes("\n")) {
                    ...
                    const issueTitle = todoMessage
                        ? `${todoMessage} in ${path}`
                        : `TODO found in ${path}`;
                    await createIssueIfNotExists(context, repo, issueTitle, path, multiLineTodoContent, multiLineTodoStart, commit, true);
                }
 ...
 */
function extractTodoMessage(line) {
 ...
 */
async function createIssueIfNotExists(context, repo, issueTitle, path, todoContent, lineIndex, commit, isMultiLine = false) {
    try {
        ...
        const issueExists = issues.some((issue) => issue.body?.includes(`<!-- todo-hash: ${todoHash} -->`));
        if (issueExists) {
...
//# sourceMappingURL=todoProcessor.js.map