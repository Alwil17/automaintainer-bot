 ...
 */
export async function loadMarketplaceConfig(context: Context): Promise<MarketplaceConfig> {
  try {
 ...
 */
export function isFeatureAvailable(feature: string, planName: string): boolean {
  const lowerPlanName = planName.toLowerCase();
  
  // Get the appropriate feature list based on plan name
  let featureList: string[] = defaultMarketplaceConfig.freeTierFeatures;
 ...
 */
export function getAvailableFeatures(planName: string): string[] {
  const lowerPlanName = planName.toLowerCase();
